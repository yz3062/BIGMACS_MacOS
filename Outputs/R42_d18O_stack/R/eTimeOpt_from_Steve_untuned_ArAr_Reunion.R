...
dat=read("Stack_untuned_uniform_age_exclusion_expanded_1172_removed_ArAr_Reunion.csv")
dat_lin = linterp(dat, dt=1)
...
amp_output = eha(dat_lin,win=win_size,fmax=0.1,ydir=-1,genplot=4,pl=2,ylab="Age (ka)",xlab="Frequency (cycles/ka)", output=2)
...
res=eTimeOpt(dat_lin,targetP=targetP,targetE=targetE,sedmin=80,sedmax=120,win=win_size,fitModPwr=F,flow=1/70,fhigh=1/25, roll=10^9, output=1,ydir=-1)			
...
# dat2=tune(dat_lin,control=control,extrapolate=T)
# 
...
# plot(dat2_lin,type="l",col="red",xlab="Age (ka)",ylab="d18O", main="red=eTimeOpt timescale        black=depth-derived timescale"); lines(dat_lin)
# 
# # this is a histogram of the temporal adjustments between the eTimeOpt and depth-derived timescales
# hist(dat2[,1]-dat_lin[,1],xlab="ka",main="Temporal adjustments")
# 
# # this is a plot of the difference between the eTimeOpt and depth-derived timescales at each datum
# plot(dat2[,1],dat2[,1]-dat_lin[,1],xlab="eTimeOpt timescale (ka)",ylab="eTimeOpt - depth derived timescale (ka)",main="Temporal adjustments")
# 
# plot(dat2[,1],dat2[,1]-dat_lin[,1]-10.0614,xlab="eTimeOpt timescale (ka)",ylab="eTimeOpt - depth derived timescale (ka)",main="Temporal adjustments"); abline(h=0,lty=2)

dat3=tune(dat_lin,control=control,extrapolate=F)
...
dat3[1]=dat3[1]+dat_lin[win_size/2+1,1]
# interpolate to even sampling grid (use median sampling interval)
...
# conduct a new eha analysis on the tuned record
amp_output_after_tuning = eha(dat3_lin,win=500,fmax=0.1,ydir=-1,genplot=4,pl=2,ylab="Age (ka)",xlab="Frequency (cycles/ka)", output=2)
...
dat_lin_trim_1 = dat_lin[-(1:win_size/2),]
dat_lin_trim_2 = dat_lin_trim_1[-(nrow(dat3)+1:nrow(dat_lin_trim_1)),]
# diff = dat3[,1] - dat[,1]
output = cbind(dat3[,1], dat_lin_trim_2[,1])
output_df = as.data.frame(output)

# save diff to excel
# library(writexl)
# write_xlsx(output_df, paste('./untuned_intermediate_tuned_ages_win',win_size,'.xlsx',sep=""))

# save data frames to excel
#### change the filename below depending on window size #####
library(writexl)
write_xlsx(dat3_lin, paste('./tuned_stack_unextrapolated_win',win_size,'.xlsx',sep=""))