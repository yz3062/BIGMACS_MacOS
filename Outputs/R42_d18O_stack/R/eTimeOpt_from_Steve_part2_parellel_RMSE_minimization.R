...
dat=read("Stack_untuned_uniform_age_exclusion_expanded_1172_removed_Hobart2023_global_interp.csv")
...
for(i in 1:nsim)
{
  ...
  for (i_slide in slide_vector){
    ...
    rmse_i = (time_lin[time_lin[,1]==781, ][,2]+i_slide-781)**2+
              ...
              (time_lin[time_lin[,1]==1180, ][,2]+i_slide-1180)**2+
              (time_lin[time_lin[,1]==1775, ][,2]+i_slide-1775)**2+
              (time_lin[time_lin[,1]==1934, ][,2]+i_slide-1934)**2+
              (time_lin[time_lin[,1]==2116, ][,2]+i_slide-2116)**2+
              (time_lin[time_lin[,1]==2140, ][,2]+i_slide-2140)**2
...
# save diff to excel
library(writexl)
write_xlsx(output_df, './untuned_intermediate_tuned_ages_uncertainty_RMSE_minimization.xlsx')
