...
dat=read("Stack_untuned_uniform_age_exclusion_expanded_1172_removed_ArAr_interp.csv")
...
{
  res=eTimeOpt(dat,targetP=targetP, targetE=c(174.8252,106.6098,97.08738),sedmin=80,sedmax=120,win=win_size,fitModPwr=F,flow=1/70,fhigh=1/25, roll=10^9, output=1,ydir=-1,genplot=F)                       	
...
for(i in 1:nsim)
{
  time[,i]=sim[[i]][,2]+win_size/2
  sr[,i]=sim[[i]][,3]
}
depth=sim[[1]][,1]

# RMSE minimization
# interpolate untuned ages with a time step of 1
for(i in 1:nsim)
{
  rmse<-double(201)
  slide_index = 1
  slide_vector = seq(from = -10, to = 10, by = 0.1)
  for (i_slide in slide_vector){
    # interpolate auto-tuned ages according to untuned ages with a time step of 1
    time_lin = linterp(cbind(depth, time[,i]),dt=1, verbose=F, genplot=F)
    # calcualte rmse for every slide of 0.1 kyr
    rmse_i = (time_lin[time_lin[,1]==781, ][,2]+i_slide-781)**2+
              (time_lin[time_lin[,1]==990, ][,2]+i_slide-990)**2+
              (time_lin[time_lin[,1]==1070, ][,2]+i_slide-1070)**2+
              (time_lin[time_lin[,1]==1186, ][,2]+i_slide-1186)**2+
              (time_lin[time_lin[,1]==1221, ][,2]+i_slide-1221)**2+
              (time_lin[time_lin[,1]==1787, ][,2]+i_slide-1787)**2+
              (time_lin[time_lin[,1]==1948, ][,2]+i_slide-1948)**2+
              (time_lin[time_lin[,1]==2606, ][,2]+i_slide-2606)**2
    rmse[slide_index] = rmse_i
    slide_index = slide_index + 1
  }
  # find the index of the smallest rmse
  # use the index to find the slide that minimizes rmse
  # add that slide to time
  time[,i] = time[,i] + slide_vector[which.min(rmse)]
}
...
# save diff to excel
library(writexl)
write_xlsx(output_df, './untuned_intermediate_tuned_ages_uncertainty_RMSE_minimization_untuned_ArAr.xlsx')
