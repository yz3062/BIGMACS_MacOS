...
for i in range(len(df_Atlantic['name'])):
    ...
    if '607_LR04' in df_Atlantic.iloc[i]['name'][0]:
        axes[1].plot(age, d18O, linestyle='--', alpha=1, label='607', color='r')
    elif '927' in df_Atlantic.iloc[i]['name'][0]:
        axes[1].plot(age, d18O, linestyle='--', alpha=1, label='927', color='y')
    elif '999' in df_Atlantic.iloc[i]['name'][0]:
        axes[1].plot(age, d18O, linestyle='--', alpha=1, label='999', color='g')
    else:
        axes[1].plot(age, d18O, alpha=0.3)
    # print(df_Atlantic.iloc[i]['name'][0] + ': ' + np.array2string(d18O[(age>1835) & (age<1850)]))
...
plt.savefig('Raw_d18O_18ma_zoom_Atlantic_heavy_d18O.png', dpi=700)