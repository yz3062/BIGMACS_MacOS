...
Pacific_path = '../../R59_d18O_stack/stack.txt'
...
columns = [n for n, v in ndata.items()]
# now make a data frame, setting the time stamps as the index
df_U1308 = pd.DataFrame(np.concatenate([ndata[c] for c in columns], axis=1),
                  columns=columns)
...
for core_name in atlantic_core_names:
    if axes_index == 6:
        d18O = ma.fix_invalid(df_U1308[df_U1308['name']==core_name]['d18O'].iloc[0]).mean(axis=1)
        shift = ma.squeeze(df_U1308[df_U1308['name']==core_name]['d18O_shift'])
        scale = ma.squeeze(df_U1308[df_U1308['name']==core_name]['d18O_scale'])
        d18O = (d18O - shift) / scale # from BIGMACS saveFigures.m
        age = ma.squeeze(df_U1308[df_U1308['name']==core_name]['median'])
        age = age[~d18O.mask.transpose()]
        d18O = d18O[~d18O.mask]
...
plt.savefig('Raw_d18O_alignment_18ma_pan_U1308_target.png', dpi=700)
