...
stack_2_path = '../../R50_d18O_stack/stack.txt'
# stack = scipy.io.loadmat(stack_path)
...
axes.plot(stack['age(kyr)'], stack['mean(permil)'], label='BIGMACS updated')
axes.fill_between(stack['age(kyr)'],
                  stack['mean(permil)']+2*stack['sigma(permil)'],
                  stack['mean(permil)']-2*stack['sigma(permil)'],
                  alpha=0.3,
                  label='BIGMACS 2sigma')
axes.plot(stack_2['age(kyr)'], stack_2['mean(permil)'], label='BIGMACS',
          color='C1')

axes.plot(LR04.index/1000, LR04, label='LRO4', color='gray', alpha=0.3,
          zorder=1)
