...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 13 18:57:24 2024
...
Indian_path = '../../R87_d18O_stack/stack.txt'
# Indian stack is really a global stack
Indian_stack = pd.read_table(Indian_path, delimiter=' ')
Indian_stack = Indian_stack.groupby('age(kyr)').mean()
...
# list of cores to plot from the Indian
Indian_core_names = ['758_LR04age']

Indian_core_display_names = [r'$ODP\ 758$']

for core_name in Indian_core_names:
    d18O = ma.fix_invalid(df_Indian[df_Indian['name']==core_name]['d18O'].iloc[0]).mean(axis=1)
    shift = ma.squeeze(df_Indian[df_Indian['name']==core_name]['d18O_shift'])
    scale = ma.squeeze(df_Indian[df_Indian['name']==core_name]['d18O_scale'])
    d18O = (d18O - shift) / scale # from BIGMACS saveFigures.m
    age = ma.squeeze(df_Indian[df_Indian['name']==core_name]['median'])
    age = age[~d18O.mask.transpose()]
    d18O = d18O[~d18O.mask]
    axes[axes_index].plot(age, d18O, 'o-', alpha=1, color='C1',markersize=2)
    axes[axes_index].plot(Indian_stack.index, Indian_stack['mean(permil)'], color='k', alpha=0.5)
    axes[axes_index].set_ylim(2.7, 4.6)
    axes[axes_index].invert_yaxis()
    axes[axes_index].text(2160, 3.8,
                          Indian_core_display_names[axes_index-4],
                          color='C1')
    axes_index += 1
...
sns.despine(ax=axes[5], top=True, bottom=False, left=False, right=True)
for i in [0, 2, 4]:
...
plt.savefig('Raw_d18O_alignment_18ma_pan_regional_s tacks_Channell2016_ODP758.pdf')