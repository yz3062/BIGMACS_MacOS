...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr  6 10:58:09 2024
...
inso_summer_65S.plot(label='SH inso.', ax=axes[6], color='C4')
...
axes[7].plot(obl.index, obl['obliquity'], color='k')
axes[7].set_ylabel('Obliquity (°)')

#%% plot CENOGRID
axes[2].plot(CENOGRID.index*1000, CENOGRID, color='k', label='CENOGRID')

#%% plot LR04
axes[3].plot(LR04.index/1000, LR04, color='k', label='LR04')

#%% plot ProbStack
axes[4].plot(ProbStack.index/1000, ProbStack, color='C3', alpha=0.3,
             label='ProbStack')
axes[4].fill_between(ProbStack.index/1000,
                     ProbStack_upper,
                     ProbStack_lower,
                     alpha=0.3,
                     color='C3')
...
axes[5].plot(Pacific_stack['age(kyr)'], Pacific_stack['mean(permil)'],
             label='Pacific stack', color='C4')
axes[5].fill_between(Pacific_stack['age(kyr)'],
                  Pacific_stack['mean(permil)']+Pacific_stack['sigma(permil)'],
                  Pacific_stack['mean(permil)']-Pacific_stack['sigma(permil)'],
                  alpha=0.3,
                  color='C4')
axes[5].plot(Atlantic_stack['age(kyr)'], Atlantic_stack['mean(permil)'],
             label='Atlantic stack', color='C5')
axes[5].fill_between(Atlantic_stack['age(kyr)'],
                  Atlantic_stack['mean(permil)']+Atlantic_stack['sigma(permil)'],
                  Atlantic_stack['mean(permil)']-Atlantic_stack['sigma(permil)'],
                  alpha=0.3,
                  color='C5')
axes[5].set_ylim(3.15, 4.4)
...
axes[3].set_ylim(3, 4.7)
axes[3].invert_yaxis()
axes[3].set_ylabel(u'$\mathrm{\delta}^\mathrm{18}$O (‰)')
axes[3].yaxis.set_label_position("right")

axes[4].set_ylim(3, 4.7)
axes[4].invert_yaxis()
axes[4].set_ylabel(u'$\mathrm{\delta}^\mathrm{18}$O (‰)')

axes[5].set_ylim(3.15, 4.4)
axes[5].invert_yaxis()
axes[5].set_ylabel(u'$\mathrm{\delta}^\mathrm{18}$O (‰)')
axes[5].yaxis.set_label_position("right")

axes[6].set_xlabel('')
axes[6].set_ylabel(r'Insolation $W/{m^2}$     ') # spaces are so labels don't overlap
axes[6].yaxis.set_label_position("right")
axes[6].yaxis.label.set_color('C4')
axes[6].spines['right'].set_color('C4')
axes[6].tick_params(axis='y', colors='C4')
axes[6].legend(loc='lower right', bbox_to_anchor=(1, -0.3))

axes[7].set_xlabel('Age (ka BP)')
...
sns.despine(ax=axes[6], top=True,right=False, left=True, bottom=True)
sns.despine(ax=axes[7], top=True,right=True, left=False, bottom=False)
