...
Atlantic_path_Wilkens = '../stack.txt'
Atlantic_stack_Wilkens = pd.read_table(Atlantic_path_Wilkens, delimiter=' ')
Atlantic_stack_Wilkens = Atlantic_stack_Wilkens.groupby('age(kyr)').mean()
...
axes[1].plot(Atlantic_stack.index, Atlantic_stack['mean(permil)'], color='C5',linestyle='--')
axes[1].plot(Atlantic_stack_Wilkens.index, Atlantic_stack_Wilkens['mean(permil)'], color='C5')
...
plt.savefig('Raw_d18O_18ma_zoom_Wilkens.png', dpi=700)